            ...
            <div class="card card-outline card-primary">
                <div class="card-header">
                    ...
                    <div class="float-right">
                        ...
                        <button type="button" class="btn btn-success" onclick="getUrl('Transaksi/form_barang_po_in/0')" <?= (($created == 1) ? _lock_button() : 'disabled') ?>><i class="fa-solid fa-circle-plus"></i>&nbsp;&nbsp;Tambah</button>
                    </div>
                </div>
...
<script>
    ...
    // fungsi cetak
    function cetak(x, y) {
        printsingle('Transaksi/single_print_bin_po/' + x + '/' + y);
    }

    //fungsi ubah berdasarkan lemparan kode
    function ubah(invoice) {
        // jalankan fungsi
        getUrl('Transaksi/form_barang_po_in/' + invoice);
    }
    ...
    function hapus(invoice) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) { // jika yakin
                ...
                // jalankan fungsi
                $.ajax({
                    url: siteUrl + 'Transaksi/delBeliPoIn/' + invoice,
                    type: 'POST',
                    dataType: 'JSON',
                    success: function(result) { // jika fungsi berjalan dengan baik

                        if (result.status == 1) { // jika mendapatkan hasil 1
                            Swal.fire("Pengajuan Pembelian", "Berhasil di hapus!", "success").then(() => {
                                reloadTable();
                            });
                        } else { // selain itu

                            Swal.fire("Pengajuan Pembelian", "Gagal di hapus!, silahkan dicoba kembali", "info");
                        }
                    },
    ...
    function valided(invoice, param) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) { // jika yakin
                ...
                // jalankan fungsi
                $.ajax({
                    url: siteUrl + 'Transaksi/accbarang_po_in/' + invoice + '/' + param,
                    type: 'POST',
                    dataType: 'JSON',
                    success: function(result) { // jika fungsi berjalan dengan baik
    ...
    function actived(invoice, param) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) { // jika yakin
                ...
                // jalankan fungsi
                $.ajax({
                    url: siteUrl + 'Transaksi/activedbarang_po_in/' + invoice + '/' + param,
                    type: 'POST',
                    dataType: 'JSON',
                    success: function(result) { // jika fungsi berjalan dengan baik
