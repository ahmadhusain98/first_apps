                            ...
                            <div class="col-md-6">
                                <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        <input type="text" class="form-control" id="kodeRole" name="kodeRole" placeholder="Otomatis" readonly>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        <input type="text" class="form-control" id="keterangan" name="keterangan" placeholder="Masukkan Role" onkeyup="ubah_nama(this.value, 'keterangan')">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="float-right">
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu">
                                <li><a class="dropdown-item" href="#" onclick="preview('role')"><i class="fa-solid fa-fw fa-tv"></i>&nbsp;&nbsp;Preview</a></li>
                                <li><a class="dropdown-item" href="#" onclick="print('role')"><i class="fa-regular fa-fw fa-file-pdf"></i>&nbsp;&nbsp;Pdf</a></li>
                                <li><a class="dropdown-item" href="#" onclick="excel('role')"><i class="fa-regular fa-fw fa-file-excel"></i>&nbsp;&nbsp;Excel</a></li>
                            </ul>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="table-responsive">
                                <table class="table table-hover table-bordered" id="tableRole" width="100%" style="border-radius: 10px;">
                                    <thead>
...
<script>
    // variable
    var table = $('#tableRole');
    const form = $('#form_role');
    var kodeRole = $('#kodeRole');
    ...
    function save() {
        ...
        if (kodeRole.val() == '' || kodeRole.val() == null) { // jika kode_role null/ kosong
            // isi param = 1
            var param = 1;
        } else { // selain itu
            // isi param = 2
        ...
        // jalankan proses cek role
        if (param == 1) {
            $.ajax({
                url: siteUrl + 'Backdoor/cekRole',
                type: 'POST',
                dataType: 'JSON',
                data: form.serialize(),
    ...
    function proses(param) {
        ...
        $.ajax({
            url: siteUrl + 'Backdoor/role_proses/' + param,
            type: "POST",
            data: form.serialize(),
            dataType: "JSON",
            success: function(result) { // jika fungsi berjalan dengan baik
                ...
                if (result.status == 1) { // jika mendapatkan respon 1
                    ...
                    Swal.fire("Role", "Berhasil " + message, "success").then(() => {
                        reseting();
                        reloadTable();
                    });
                } else { // selain itu

                    Swal.fire("Role", "Gagal " + message + ", silahkan dicoba kembali", "info");
                }
            },
            error: function(result) { // jika fungsi error
                btnSimpan.attr('disabled', false);
    ...
    function ubah(kode_role) {
        // jalankan fungsi
        $.ajax({
            url: siteUrl + 'Backdoor/getInfoRole/' + kode_role,
            type: 'POST',
            dataType: 'JSON',
            success: function(result) {
                if (result) { // jika hasilnya ada, isi form dengan hasil
                    kodeRole.val(kode_role);
    ...
    // fungsi hapus berdasarkan kode_role
    function hapus(kode_role) {
        // ajukan pertanyaaan
        Swal.fire({
            title: "Kamu yakin?",
            ...
            if (result.isConfirmed) { // jika yakin
                ...
                // jalankan fungsi
                $.ajax({
                    url: siteUrl + 'Master/delRole/' + kode_role,
                    type: 'POST',
                    dataType: 'JSON',
                    success: function(result) { // jika fungsi berjalan dengan baik
                        btnSimpan.attr('disabled', false);
                        ...
                        if (result.status == 1) { // jika mendapatkan hasil 1
                            ...
                            Swal.fire("Role", "Berhasil di hapus!", "success").then(() => {
                                reloadTable();
                            });
                        } else { // selain itu

                            Swal.fire("Role", "Gagal di hapus!, silahkan dicoba kembali", "info");
                        }
                    },
                    error: function(result) { // jika fungsi error
                        btnSimpan.attr('disabled', false);
    ...
    function showGuide() {
        ...
        // isi text
        $('#modal_mgLabel').append(`Manual Guide Backdoor Role`);
        $('#modal-isi').append(`
            <ol>
